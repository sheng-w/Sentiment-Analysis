...
#data_raw <- read.csv("/Users/wang/Box Sync/my/k/CS760/project/Reviews.csv") #read the data
counts = table(data_raw$Score) #unbalanced
indexDrop = sample(which(data_raw$Score == 5),counts[5]/2)
data_new = data_raw[-indexDrop,]

##get the train and test, and validation sets

data_raw_subset <- data_new[1:20000,]
#data_raw_subset <- data_new[20001:40000,]
...
test = data_raw[(dim(data_raw)[1] - test_size + 1):dim(data_raw)[1],c("Summary", "Text", "Score")]
validation = data[(train_size + test_size + 1):data_size,]
...
data_tree = data_raw[(dim(data_raw)[1] - test_size + 1):dim(data_raw)[1],c("HelpfulnessNumerator", "HelpfulnessDenominator")]
data_tree_rate = 0
data_tree_size = dim(data_tree)[1]
for(i in 1:data_tree_size)
{
...
#data_tree_all = cbind(data_tree, feature_neural_text_test)
#data_tree_all = cbind(feature_neural_text_test)
